...
class TrickyStore : SettingsPreferenceFragment() {
    ...
    private fun refreshStatus() {
        ...
        findPreference<Preference>("ts_manage_targets")?.summary =
            if (targetCount > 0) {
                ...
                getString(R.string.ts_target_apps_count_with_mode, targetCount, mode)
    ...
    private fun readCurrentMode(): String {
        ...
        return when {
            firstLine.trimEnd().endsWith("!") -> getString(R.string.ts_verification_mode_cert)
            firstLine.trimEnd().endsWith("?") -> getString(R.string.ts_verification_mode_leaf)
            else -> getString(R.string.ts_verification_mode_auto)
        }
    }
