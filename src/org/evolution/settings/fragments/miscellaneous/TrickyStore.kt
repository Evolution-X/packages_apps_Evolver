...
class TrickyStore : SettingsPreferenceFragment() {
    ...
    private val backStackListener = FragmentManager.OnBackStackChangedListener {
        if (parentFragmentManager.backStackEntryCount == 0) refreshStatus()
    }
    ...
    private fun showTargetAppPicker() {
        parentFragmentManager.beginTransaction()
            .replace(id, TrickyStoreAppSettings())
            .addToBackStack(null)
            .commit()
    }
    ...
    private fun seedDefaultTargetsIfEmpty() {
        val existing = Settings.Secure.getString(requireContext().contentResolver, TARGET_KEY)
        if (!existing.isNullOrEmpty()) return
        Settings.Secure.putString(
            requireContext().contentResolver,
            TARGET_KEY,
            TrickyStoreAppSettings.DEFAULT_TARGETS.joinToString("\n")
        )
    }
