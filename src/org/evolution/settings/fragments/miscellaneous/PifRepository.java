...
public class PifRepository {
    ...
    private static final String EVOLUTION_PIF_URL =
    ...
    public PifResult fetchEvolutionPif() {
        try {
            String response = fetchUrl(EVOLUTION_PIF_URL);
            JSONObject json = new JSONObject(response);
            sanitizeEvolutionPif(json);
            String model = json.optString("MODEL", "Evolution X profile");
            return new PifResult.Success(model, json);
        } catch (Exception e) {
            Log.e(TAG, "Failed fetching Evolution X PIF", e);
            return new PifResult.Error("Failed to fetch from Evolution X: " + e.getMessage(), e);
        }
    }

    private void sanitizeEvolutionPif(JSONObject json) {
        try {
            json.put("spoofVendingSdk", "false");
            json.remove("SDK_INT");
        } catch (Exception e) {
            Log.w(TAG, "Failed to sanitize Evolution X PIF", e);
        }
    }
